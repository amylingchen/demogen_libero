...
| 7.3 seg id 表 + cabinet 拆 middle/top drawer 部件级 id | ✅ **已实现**（`goal_scene.GOAL_SEG_IDS` / `build_goal_seg_lut` / `orientation_of` / `flip_fingerprint`；10 实体全覆盖，抽屉部件独立 id 110/120）。可行性证据 | `scripts/probe_drawer_seg.py` → `output/goal_drawer_seg/drawer_seg_probe.json`：instance 级只有整体 `wooden_cabinet_1`（11 个 id），但 **element 级（38 个 id）按 `geom_bodyid` 归组可得部件掩码**。关闭状态下 middle=448px、top=593px、bottom=377px，均高于 60px 门槛，初始帧可接地。关节↔body 命名逐个验证正确（`{lvl}_level` 驱动 `cabinet_{lvl}`，y 由 −0.245 移到 −0.085）。开中间抽屉时 `cabinet_middle` 448→2289→4423px（半开/全开），`cabinet_top` 593→563→621px 基本不动，`cabinet_bottom` 377→53→61px（被拉出的抽屉遮住）——**打开的抽屉自己变大，邻居不会暴涨**。**仍然不可用"柜体部件里最大的掩码"来选指令所指的抽屉**，但理由不是邻居暴涨，而是**初始帧（两抽屉都关）top 就比 middle 大**：探针里 593 vs 448，渲染数据里 20/20 条 demo 都是 top 更大（462±225 vs 380±171 px）——初始帧就会选错。

> **2026-08-26 更正。** 本行原先记的是 middle=412 / top=469 / 开中间抽屉使 top 涨到 4249px，并据此写了"静止抽屉掩码被邻居撑爆"的陷阱。那组数出自 `probe_drawer_seg.py` 的 `geom_bodyid[v-1]`，与 `build_goal_seg_lut` 的 `lut[g+1]` 是同一个 off-by-one：robosuite 只在 instance/class 级做 `mapping.get(x,-1)+1`，**element 级返回的是 0 基原始 geom 索引**，两处都按 +1 消费，于是每个 geom 拿到前一个 geom 的标签。所谓"top 暴涨"其实就是中间抽屉自己被打开、却被贴上了 top 的标签——**该陷阱不存在**。旧值存档在 `drawer_seg_probe.json.bak_offbyone_20260826`。同一个 bug 也污染过 `goal_oc_v3` 的全部 seg（碗完全无掩码），已随 `goal_oc_v4` 重渲修复；修法是把 +1 收进 `goal_scene.map_element_seg` 一个函数，调用点不再自己索引 LUT |
| 7.4 object_geometry.json | ✅ **已做** | `scripts/dump_goal_geometry.py` → `output/goal_gen_v3/object_geometry.json`。按**网格顶点**算 AABB（粗略的"最大半边长当球半径"版本把酒瓶算成 0.21m 宽、酒架 0.57m，已弃用）。实测：碗 0.111×0.112×0.053、奶酪 0.083×0.043×0.019、瓶 0.043×0.044×0.159、盘 0.138×0.138×0.019；柜顶面 z=1.128、三抽屉顶 1.124/1.054/0.984；酒架 0.27×0.266×0.33。**独立交叉验证**：按官方布局标定的 `OBJECT_RADIUS` 与实测网格半宽比值 碗 1.07 / 奶酪 0.97 / 盘 1.01（瓶 1.81，保守无害）——标定不是"凑合能容纳 L00"，而是physically 对的。柜顶实测放置高度比模型表面高 +0.032m（末端在表面上方释放，符合预期） |
